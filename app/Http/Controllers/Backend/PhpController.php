...
class PhpController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if($validator->fails()){
            return redirect('/backend/competition/php/edit/'.$id)->with(['data' => $inputs])->withErrors($validator);
        }
        ...
        $php->save();
        return redirect('/backend/competition/php/view/'.$id);
    }
    ...
    public function excel() {
        return Excel::create('php_competition_export_' . time(), function($excel)
        {
            ...
            {
                $datas = Php::all();
                $sheet->loadView('backend.competition.excel.php', ['datas' => $datas]);
            });
        })->download('xls');
    }
}
