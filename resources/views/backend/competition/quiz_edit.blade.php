...
@section('content')
    <section class="regis-error">
        ...
        @endif
        <form action="{{URL::route('backend.competition.quiz.update', $data->id)}}" method="post">
            {!! csrf_field() !!}
            {{ method_field('PUT') }}
            {{--ทีม--}}
            <h3>ชื่อทีม</h3>
                ...
                <div class="tab-pane fade" id="students">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-8 col-xs-6">
                            <div class="form-group">
                                <label>อีเมล</label>
                                <input type="email" class="form-control" placeholder="e.g. openhouse@it.kmitl.ac.th" name="email[]" required value="{{ $data['email'][2] }}">
                            </div>
                        </div>
                    </div>
                </div>
